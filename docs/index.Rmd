...
### `r fontawesome::fa("save")` `{targets}` data store
...
- It is not recommended to directly interact with the `_targets` directory. Instead, load and inspect the direction using the suite of available helper functions
  + `tar_read()`/`tar_load()`
  + `tar_meta()`
  + Etc.
...
### `r fontawesome::fa("robot")` A simple example...
...
- If your project lives on our cluster, you can change the scheduler to `"slurm"`, and `targets` will run the worker processes as jobs on the compute nodes
...
- Now these worker processes will run on compute nodes (caveat: currently few nodes actually support `ZeroMQ` but this should be made widely available next upgrade)
...
- Using the `{future}` backend for parallel computing (I personally prefer the `{clustermq}` backend)
