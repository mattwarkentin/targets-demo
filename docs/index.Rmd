...
```{r echo=FALSE}
htmltools::img(src = "imgs/drake.svg", class = "drake-pic")
```
...
--
  ...
  - `tar_make()` - Analyzes your code/pipeline to understand the dependencies, and builds the pipeline in a separate clean `R` session
...
- Name the script the name of the function it contains
...
- Let's fit a model for how life expectancy has changed over time for every country in the `gapminder` data set
