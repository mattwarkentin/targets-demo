...
Hi, this is Mauro . I hope you and your loved ones are safe. __I'm writing to ask if you would be willing to review a package for rOpenSci.__ As you probably know, rOpenSci conducts peer review of R packages contributed to our collection in a manner similar to journals.

__The package targets by Will Landau provides make-like pipelines for R.__ targets supersedes drake, and is submitted to rOpenSci jointly with the package tarchetypes. You can find targets and tarchetypes on GitHub here and here. We conduct our open review process via GitHub as well.
...
.fr.f4.gray[See the [Statement of Need](https://wlandau.github.io/targets/articles/need.html) for details.]
...
```{r echo=FALSE}
img(src = "imgs/4lp7k8.jpg", class = "w-50")
```
...
- All functions in `{targets}` are prefixed by `tar_*`, which makes it easy to work with the package due to low cognitive burden
...
--
  ...
  - `tar_pipeline()` - The complete set of all targets included in the analyze
...
--
  ...
  - `tar_make()` - This function will analyze your code/pipeline to understand the dependencies, and build the pipeline in a separate clean `R` session
...
### `r fontawesome::fa("list")` A move away from imperative scripting
...
- Building your plan
  ...
  + A `{targets}` plan is a __directed acyclic graph__ that describes all of the tasks that need to be done and the interrelationships between these tasks
...
- A key design consideration when working with `{targets}` is to embrace functions. 
    + Try to abstract important tasks into functions that do a single obvious task
...
```r
find_lung_cancers <- function(data, icd_code) {
  # <<some R code>>
  return(data_with_outcomes)
}
```
```r
find_lung_cancers(ukb, "C34")
```
...
- A more mature project will likely have many more subdirectories and files, but this serves as the skeleton for most/all of my `{targets}` projects

However,

- `_targets.R` and `_targets/` must exist at the root of the project
...
- I find it best to have one script per function/task; name the script the name of the function it contains
...
- It is not recommended to directly interact with the `_targets` directory. Instead, load and inspect the data store using the suite of available helper functions
...
__Patterns:__ Creates .i.blue[branches] based on combinations of arguments
...
- By default, `targets` will aggregate each of the slices of `size` using `vctrs::vec_c()`. In this example, this will combine all of our draws into one single vector.

---

### `r fontawesome::fa("tree")` Dynamic branching

__Iteration__: patterns iterate over targets, and there are two important aspects of iteration...

1. Branching
  + This tells `targets` how to iterate (or create branches) based on the data

1. Aggregation
  + This tells `targets` how to combine branches to form targets

--

- `iteration = "vector"` - branches are created with `vec_slice()` and aggregated with `vec_c()`

- `iteration = "list"` - branches are created with `list[[]]` and aggregated with `list()`
...
- Let's fit a linear model for how life expectancy has change over time for every country in the world
  + How many countries are in our data set? Not sure, doesn't matter
  
- New concept: __dynamic branching__
...
```{r echo=FALSE}
img(src = "imgs/gapminder.png", class = "center")
```
...
```{r echo=FALSE}
img(src = "imgs/gapminder2.png", class = "center")
```

---

### `r fontawesome::fa("project-diagram")` High-performance Computing
...
- Assuming you have installed the `{clustermq}` package...enabling parallel processing of targets is as simple as...
...
```r
# Instead of tar_make()
targets::tar_make_clustermq(workers = 5)
```
...
- If your project lives on our cluster NFS, you can change the scheduler to `"slurm"`, and `targets` will run the worker processes as jobs on the compute nodes
...
- Now these worker processes will run on compute nodes (.i[caveat: currently few nodes actually support `ZeroMQ` but this should be made widely available next upgrade])
...
- [tarchetypes Website](https://wlandau.github.io/tarchetypes/)
]
...
<br><br><br>
.ba.pa3.br3.bg-yellow.f4.center[.b[Kohn's Second Law:] An experiment is reproducible until another laboratory tries to repeat it]
<br><br>
.ba.pa3.br3.bg-light-green.f4.center[.b[Matt's Second Law:] A statistical analysis is reproducible until you try to re-run it ~~years~~ days later]
