...
--
  ...
  - `tar_pipeline()` - The complete set of all targets are contained in the pipeline
...
```{r echo=FALSE}
div(".b[NOTE:] <code>tar_read()</code> reads an object into memory, but the user must assign the object into a variable for persistence; <code>tar_load()</code> reads and assigns the object into a variable of the same name.", class = "f5 ba pa1 br3 bg-light-yellow")
```
